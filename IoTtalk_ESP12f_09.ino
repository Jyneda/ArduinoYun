...
#define DM_name  "UV-Light" 
#define DF_list  {"Switch-O", "Motion", "State"}
...
void handleRoot(){
  ...
  temp += "<input type=\"button\" value=\"關閉\" style=\"width:100px; height:100px; background-color:red\" onclick=\"location.href=\'turn_off_pin\'\"><br></div>";
  temp += "</html>";
...
void setup() {
    ...
    pinMode(4, OUTPUT);//GPIO16 : relay signal
    digitalWrite(4,LOW);
    ...
    while(wifimode){
        ...
        if (millis() - cycleTimestamp > 500){  // Wi-Fi AP malfunction after intensive analogRead(A0). 
            sensorValue = analogRead(A0);      // Do not intensively read A0 to keep AP working.
            cycleTimestamp = millis();
        }
        if (sensorValue > 800) suspend = millis();
        if ( (millis()-suspend > 10000) && switchFlag) digitalWrite(4,HIGH);
...
int LED_flag = 0;
String result;
int Motion, previousMotion;
int previousSwitchState = 99;
int switchState=0;
void loop() {
    if (digitalRead(5) == LOW) clr_eeprom();
    ...
    if (millis() - cycleTimestamp > 500) {
        ...
        if (result != "___NULL_DATA___"){
            Serial.println ("Switch-O Data: "+result);
            if (result.toInt() > 0) switchFlag = 1;                      
            else if(result.toInt() == 0) switchFlag = 0;
        }
    ...
    if ((millis()-suspend > 10000) && switchFlag){
        digitalWrite(4,HIGH);
        switchState = 1;
    }
    else{
